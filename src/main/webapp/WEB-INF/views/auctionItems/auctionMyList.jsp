...
<body>
	...
	<div class="row">
		<div class="span4 offset7">
			...
			<div><a class="btn" href="${ctx}/auction/user/create">添加个人拍卖物品</a></div> 
				...
				<tr>
					...
					<c:if test="${task.number != 0}">
						<c:if test="${task.isAuction eq 'Y'}">
							<c:if test="${task.status eq 'Y'}"><a href="${ctx}/auctionUser/user/create/${task.id}" >参与竞拍</a>&nbsp;</c:if> 
						</c:if>
						<c:if test="${task.isAuction != 'Y'}"><a href="${ctx}/auctionApply/user/create/${task.id}" >购买</a>&nbsp;</c:if>
